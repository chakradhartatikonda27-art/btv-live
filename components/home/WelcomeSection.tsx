    ...
    <section style={{ padding: '60px 20px', background: 'linear-gradient(180deg, #08090B 0%, #0D0F12 50%, #08090B 100%)', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: '40px' }}>
          ...
          <h2 style={{ fontFamily: 'Georgia, serif', fontSize: 'clamp(24px, 4vw, 48px)', color: '#EDEEF0', margin: '0 0 16px', lineHeight: 1.2, letterSpacing: '-0.02em' }}>
            {title.includes('&') ? (
              <>
                {title.split('&')[0]}&<br />
                <span style={{ color: '#D4A832' }}>{title.split('&')[1]}</span>
              </>
            ) : title}
          </h2>

          <p style={{ color: '#9A9DA5', fontSize: 'clamp(13px, 2vw, 16px)', lineHeight: 1.7, maxWidth: '680px', margin: '0 auto 24px' }}>
            {desc}
          </p>
        ...
        <div style={{ textAlign: 'center', padding: '32px', borderRadius: '20px', background: 'linear-gradient(135deg, #0D0F12, #141619)', border: '1px solid rgba(212,168,50,0.15)', position: 'relative', overflow: 'hidden' }}>
          ...
          <p style={{ color: '#7A7D85', fontFamily: 'monospace', fontSize: '11px', letterSpacing: '0.2em', textTransform: 'uppercase', margin: '0 0 12px' }}>Our Mission</p>
          <p style={{ color: '#D4D6DA', fontSize: 'clamp(14px, 2vw, 20px)', fontFamily: 'Georgia, serif', fontStyle: 'italic', lineHeight: 1.6, margin: '0 0 16px', maxWidth: '700px', marginLeft: 'auto', marginRight: 'auto' }}>
            "{mission}"
          </p>
          <p style={{ color: '#D4A832', fontFamily: 'monospace', fontSize: '12px', fontWeight: '700', margin: 0, letterSpacing: '0.1em' }}>
