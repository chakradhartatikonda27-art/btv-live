...
export default function EditBusinessPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    params.then(p => {
      setBizId(p.id);
      fetch('/api/admin/directory/' + p.id).then(r => r.json()).then(d => {
        if (d.business) {
          const b = d.business;
          setForm({
            ...
            featured: b.featured, verified: b.verified,
          });
        }
        setFetching(false);
      });
  ...
  return (
    <div style={{ padding: '32px', maxWidth: '700px' }}>
      <div style={{ marginBottom: '32px' }}>
        ...
        <h1 style={{ color: '#EDEEF0', fontSize: '28px', fontFamily: 'Georgia, serif', margin: 0 }}>Edit Business</h1>
      </div>
      <form onSubmit={handleSubmit}>
        <div style={{ background: '#141619', border: '1px solid #252830', borderRadius: '12px', padding: '24px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div><label style={labelStyle}>Business Name *</label><input type='text' value={form.name} onChange={e => update('name', e.target.value)} required style={inputStyle} /></div>
          <div><label style={labelStyle}>Category</label>
            <select value={form.category} onChange={e => update('category', e.target.value)} style={{ ...inputStyle, cursor: 'pointer' }}>
              {CATEGORIES.map(c => <option key={c} value={c}>{c.replace('_',' ')}</option>)}
            </select>
          </div>
          <div><label style={labelStyle}>Description</label><textarea value={form.description} onChange={e => update('description', e.target.value)} rows={3} style={{ ...inputStyle, resize: 'none' as const }} /></div>
          <div><label style={labelStyle}>Address</label><input type='text' value={form.address} onChange={e => update('address', e.target.value)} style={inputStyle} /></div>
          ...
          <div><label style={labelStyle}>Cover Image URL</label><input type='url' value={form.coverImage} onChange={e => update('coverImage', e.target.value)} style={inputStyle} /></div>
