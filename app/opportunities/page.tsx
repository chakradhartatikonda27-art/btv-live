    ...
    <main style={{ minHeight: '100vh', background: 'var(--bg-primary)', paddingTop: '80px', paddingBottom: '60px' }}>
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: '0 16px' }}>
        ...
        <div style={{ marginBottom: '32px' }}>
          ...
          <h1 style={{ color: 'var(--text-primary)', fontSize: 'clamp(28px, 5vw, 48px)', fontFamily: 'Georgia, serif', margin: '0 0 8px' }}>Opportunity Feed</h1>
          <p style={{ color: 'var(--text-subtle)', fontSize: '14px', margin: 0 }}>Jobs, business leads, tenders and partnerships across AP and Telangana</p>
        </div>
        ...
        {loading ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} style={{ padding: '20px', borderRadius: '12px', background: 'var(--bg-card)', border: '1px solid var(--border-color)', height: '100px' }} />
            ))}
          </div>
          ...
          <div style={{ textAlign: 'center', padding: '80px 20px', borderRadius: '12px', background: 'var(--bg-card)', border: '1px solid var(--border-color)' }}>
            <p style={{ color: 'var(--text-subtle)', fontSize: '16px', margin: 0 }}>No opportunities listed yet.</p>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {opportunities.map((opp) => {
              ...
              return (
                <div key={opp.id} className='btv-card' style={{ padding: '20px', borderRadius: '12px', background: 'var(--bg-card)', border: '1px solid var(--border-color)' }}>
                  <div style={{ display: 'flex', gap: '16px', alignItems: 'flex-start' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '12px', flexWrap: 'wrap', marginBottom: '6px' }}>
                        <div>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                            ...
                            {opp.featured && (
                              <span style={{ fontSize: '10px', fontWeight: '600', color: 'var(--accent-gold)', background: 'rgba(212,168,50,0.1)', padding: '1px 6px', borderRadius: '4px' }}>Featured</span>
                            )}
                          </div>
                          <h3 style={{ color: 'var(--text-primary)', fontSize: '16px', fontWeight: '600', margin: 0, fontFamily: 'Georgia, serif' }}>{opp.title}</h3>
                          {opp.company && <p style={{ color: 'var(--text-muted)', fontSize: '13px', margin: '2px 0 0' }}>{opp.company}</p>}
                        </div>
                      ...
                      <p style={{ color: 'var(--text-subtle)', fontSize: '13px', lineHeight: 1.5, margin: '8px 0', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' as any, overflow: 'hidden' }}>
                        {opp.description}
                      </p>

                      <div style={{ display: 'flex', alignItems: 'center', gap: '16px', flexWrap: 'wrap', marginTop: '10px' }}>
                        {(opp.city || opp.state) && (
                          <div style={{ display: 'flex', alignItems: 'center', gap: '4px', color: 'var(--text-faint)', fontSize: '12px' }}>
                            <MapPin size={12} />
                        ...
                        <div style={{ marginLeft: 'auto', display: 'flex', gap: '8px' }}>
                          {opp.applyEmail && (
                            <a href={'mailto:' + opp.applyEmail} style={{ display: 'flex', alignItems: 'center', gap: '6px', color: 'var(--text-muted)', fontSize: '12px', textDecoration: 'none', padding: '6px 12px', borderRadius: '999px', border: '1px solid var(--border-color)' }}>
                              <Mail size={12} /> Email
                            </a>
                          )}
                          {opp.applyUrl && (
                            <a href={opp.applyUrl} target='_blank' rel='noopener noreferrer' style={{ display: 'flex', alignItems: 'center', gap: '6px', color: '#08090B', fontSize: '12px', fontWeight: '600', textDecoration: 'none', padding: '6px 14px', borderRadius: '999px', background: 'var(--accent-gold)' }}>
